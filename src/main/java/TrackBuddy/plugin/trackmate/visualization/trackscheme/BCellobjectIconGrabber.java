...
/**
 * This class is used to take a snapshot of a {@link BCellobject} object (or
 * collection) from its coordinates and an {@link ImagePlus} that contain the
 * pixel data.
 *
 * @author Jean-Yves Tinevez - Dec 2010 - 2017
 */
public class BCellobjectIconGrabber< T extends RealType< T >>
{

	private final ImgPlus< T > img;

	public BCellobjectIconGrabber( final ImgPlus< T > img )
	{
		this.img = img;
	}

	/**
	 * Returns the image string for the specified BCellobject. The BCellobject x,y,z and
	 ...
	 *
	 * @param BCellobject
	 *            the BCellobject to generate a thumbnail image from.
	 * @param radiusFactor
	...
	public String getImageString( final BCellobject BCellobject, final double radiusFactor )
	{
		...
		{
			slice = Math.round( BCellobject.getFeature( BCellobject.POSITION_Z ) );
			if ( slice < 0 )
			{
				slice = 0;
			}
