package TrackBuddy.plugin.trackmate.features.edges;
			...
			threads[ i ] = new Thread( "EdgeTimeLocationAnalyzer thread " + i )
			{
				...
				public void run()
				{
					...
					while ( ( edge = queue.poll() ) != null )
					{
						...
						final double x = 0.5 * ( source.getFeature( BCellobject.POSITION_X ) + target.getFeature( BCellobject.POSITION_X ) );
						final double y = 0.5 * ( source.getFeature( BCellobject.POSITION_Y ) + target.getFeature( BCellobject.POSITION_Y ) );
						final double z = 0.5 * ( source.getFeature( BCellobject.POSITION_Z ) + target.getFeature( BCellobject.POSITION_Z ) );
						final double t = 0.5 * ( source.getFeature( BCellobject.POSITION_T ) + target.getFeature( BCellobject.POSITION_T ) );
