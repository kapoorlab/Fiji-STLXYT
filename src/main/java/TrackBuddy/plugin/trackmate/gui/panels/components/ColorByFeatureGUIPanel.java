package TrackBuddy.plugin.trackmate.gui.panels.components;
...
import TrackBuddy.plugin.trackmate.features.manual.ManualEdgeColorAnalyzer;
import TrackBuddy.plugin.trackmate.features.manual.ManualBCellobjectColorAnalyzerFactory;
import TrackBuddy.plugin.trackmate.gui.GuiUtils;
import TrackBuddy.plugin.trackmate.gui.panels.ActionListenablePanel;
import TrackBuddy.plugin.trackmate.visualization.MinMaxAdjustable;
...
public class ColorByFeatureGUIPanel extends ActionListenablePanel implements MinMaxAdjustable
{
	...
	{
		BCellobjectS( "BCellobjects" ), EDGES( "edges" ), TRACKS( "tracks" ), DEFAULT( "Default" );
	...
	private void repaintColorCanvas( final Graphics g )
	{
		if ( null == jComboBoxSetColorBy.getSelectedItem()
				...
				|| ( getColorGeneratorCategory().equals( Category.BCellobjectS ) && getColorFeature().equals( ManualBCellobjectColorAnalyzerFactory.FEATURE ) )
	...
	protected CategoryJComboBox< Category, String > createComboBoxSelector( final List< Category > lCategories )
	{
		...
		for ( final Category category : lCategories )
		{
			...
			{
			case BCellobjectS:
				categoryNames.put( Category.BCellobjectS, "BCellobject features:" );
				final Collection< String > BCellobjectFeatures = new ArrayList< >( model.getFeatureModel().getBCellobjectFeatures() );
				features.put( Category.BCellobjectS, BCellobjectFeatures );
	...
	protected double[] getValues( final CategoryJComboBox< Category, String > cb )
	{
		...
		case BCellobjectS:
			...
			values = BCellobjects.collectValues( feature, false );
			break;
		case DEFAULT:
