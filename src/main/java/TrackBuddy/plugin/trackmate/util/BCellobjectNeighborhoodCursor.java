...
public class BCellobjectNeighborhoodCursor<T extends RealType<T>> implements Cursor<T> {
	...
	public BCellobjectNeighborhoodCursor(BCellobjectNeighborhood<T> sn) {
	...
	/**
	 * Return the current inclination with respect to this BCellobject center. Will be in
	 * the range [0, π]. 
	 * <p>
	...
	public double getTheta() {
		if (numDimensions() < 2)
			return 0;
		double dx = ( cursor.getDoublePosition(2) - center[2]);
	...
	public double getPhi() {
		double dx = ( cursor.getDoublePosition(0) - center[0]);
		double dy =  ( cursor.getDoublePosition(1) - center[1]);
