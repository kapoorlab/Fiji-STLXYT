package TrackBuddy.plugin.trackmate.util;
...
public class BCellobjectNeighborhood< T extends RealType< T >> implements Neighborhood< T >
{
	...
	public BCellobjectNeighborhood( final BCellobject BCellobject, final ImgPlus< T > img )
	{
		// Center
		this.center = new long[ img.numDimensions() ];
		for ( int d = 0; d < center.length; d++ )
		{
			center[ d ] = Math.round( BCellobject.getFeature( BCellobject.POSITION_FEATURES[ d ] ).doubleValue() );
		}
		// Span
		final long[] span = new long[ img.numDimensions() ];
		for ( int d = 0; d < span.length; d++ )
		{
			span[ d ] = Math.round( BCellobject.getFeature( BCellobject.Size ) );
		}
	...
	@Override
	public final BCellobjectNeighborhoodCursor< T > cursor()
	{
		return new BCellobjectNeighborhoodCursor< >( this );
	}

	@Override
	public BCellobjectNeighborhoodCursor< T > localizingCursor()
	{
		return cursor();
	}
	...
	@Override
	public BCellobjectNeighborhoodCursor< T > iterator()
	{
		return cursor();
	}
